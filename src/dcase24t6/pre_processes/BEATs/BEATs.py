...
from dcase24t6.pre_processes.BEATs.backbone import TransformerEncoder
...
class BEATs(nn.Module):
    ...
    def extract_features(
        ...
        if (
        ...
        ):  # TODO: Make sure whether or not the mask should be used or not
            ...
            return (
                ...
                padding_mask,
                embeddings,
            )  # Logits:torch.Size([1, 1304, 768])  Embs: torch.Size([1, 768])
        else:
            return x, padding_mask, embeddings

    def forward(self, cfg):
        source = cfg["audio"]

        if self.resample is None:
            self.resample = Resample(orig_freq=cfg["sr"], new_freq=16_000)

        source = self.resample(source.to("cpu"))
        cfg["audio"] = source  # torch.Size([1, 418622])
        cfg["sr"] = 16_000

        working_source = source.squeeze(0)
        num_points = source.shape[1]

        values = []
        counter = 0
        while counter + self.window_size < num_points:
            values.append(
                working_source[counter : counter + self.window_size].unsqueeze(0)
            )
            counter += int(self.window_size * 0.2)

        values = torch.cat(values)

        _, _, logits = self.extract_features(values.to("cuda"))

        cfg["frame_embs"] = logits.transpose(-1, -2)
        cfg["frame_embs_shape"] = cfg["frame_embs"].shape
